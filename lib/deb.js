...
Deb.prototype.pack = function (definition, files, callback) {
    ...
    async.series([
        this.packFiles.bind(this, files, definition),
        this.buildControlFile.bind(this, definition),
        function buildDebBinFile(done) {
            fs.writeFile(getPath('debian-binary', definition), '2.0\n', done);
        },
        function buildPackage(done) {
            var pkgName = definition.info.pkgName ||
 ...
 */
Deb.prototype.buildControlFile = function(definition, callback) {
    var self = this;
    ...
    self.controlFile = {
        ...
        Description: (definition.info.appName || '') + '\n ' + definition.package.description
    };
 ...
 */
Deb.prototype.packFiles = function(files, definition, callback) {
    var self = this;
        ...
        async.waterfall([
            ...
            function (stats, wtfDone) {
                if (stats.isDirectory()) {
                    ...
                    addParentDirs(self.data, path.dirname(crtFile.dest), self.dirs, function () {
                        async.waterfall([
...
function addParentDirs(tarball, dir, createdDirs, callback) {
    ...
    function addDir() {
        if (!createdDirs[dir]) {
            createdDirs[dir] = 1;
            tarball.entry({name: '.' + dir, type: 'directory', uname: 'root', gname: 'root' }, callback);
        } else {
            callback();
        }
    }
